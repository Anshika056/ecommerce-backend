...
exports.signup = async (req,res) => {                  //to get the data(The data sent to the server with POST is stored in the request body of the HTTP request:)
    ...
    try{
         const userexists = await User.findOne({email:email})    // check if email already exists in database
          ...
          const user = new User({name, email , phone , password, confirmpassword,role :"admin"});        // new data filled will be saved in user
         
          const signup = await user.save();
         if(signup){
           res.status(201).json({message:"Admin Registation sucessful"});                     // if data is saved to the  mongo then this will happen
         }
